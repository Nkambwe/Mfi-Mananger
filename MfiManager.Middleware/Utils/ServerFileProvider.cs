...
namespace MfiManager.Middleware.Utils {
    public class ServerFileProvider(IWebHostEnvironment webHostEnvironment)
        : PhysicalFileProvider(File.Exists(webHostEnvironment.ContentRootPath) ?
                  Path.GetDirectoryName(webHostEnvironment.ContentRootPath) :
                  webHostEnvironment.ContentRootPath), IServerFileProvider {
        protected string WebRootPath { get; } = File.Exists(webHostEnvironment.WebRootPath)
