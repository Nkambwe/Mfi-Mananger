using MfiManager.Middleware.Server.Defaults;
...
namespace MfiManager.Middleware.Data.Services {
    ...
    public class ServiceLocalization(
        ILogger<ServiceLocalization> logger,
        IServerWebHelper webHelper,
        IHttpContextAccessor httpContextAccessor,
        IServerFileProvider _fileProvider) : IServiceLocalization {

        private IList<ServerLanguage> _availableLanguages;
        private readonly ILogger<ServiceLocalization> _logger = logger;
        private readonly IHttpContextAccessor _httpContextAccessor = httpContextAccessor;
        private readonly IServerWebHelper _webHelper = webHelper;
        private readonly IServerFileProvider _fileProvider = _fileProvider;

        public IList<ServerLanguage> GetAvailableLanguages() {
            try {
                ...
                foreach (var filePath in _fileProvider.EnumerateFiles(_fileProvider.MapPath("~/Localization/"), "*.xml")) {
                    ...
                    var r = new Regex(Regex.Escape("server-language.") + "(.*?)" + Regex.Escape(".xml"));
                    ...
                    //create language
                    var language = new ServerLanguage {
                        Code = languageCode,
                        Name = languageName,
                        IsDefault = isDefault,
                    ...
                    foreach (XmlNode resNode in resources) {
                        ...
                        language.Resources.Add(new ServerLanguageResource {
                            Name = resourceName,
                            Value = resourceValue
                        });
                    }
        ...
        public string GetBrowserCulture() {
            ...
            return userLanguages.FirstOrDefault()?.Split(',').FirstOrDefault() ?? GeneralDefaults.DefaultLanguageCulture;
        }
