...
namespace MfiManager.Middleware.Logging {
    /// <summary>
    /// Class handles middleware logging
    /// </summary>
    /// <remarks>
    /// Usage: logging levels
    /// _logger.LogError("Something bad happened");
    ///_logger.LogWarning("Something suspicious happened");
    ///_logger.LogDebug("Debug details here");
    ///_logger.LogInformation("Just FYI: {Value}", 42);
    ///
    /// Shorthand extensions:
    /// _logger.LogError(ex, "Error occurred");
    ///_logger.LogWarning("Warning about {Thing}", thing);
    ///_logger.LogDebug("Debugging info {Detail}", detail);
    ///_logger.LogInformation("Info message {Id}", id);
    /// </remarks>
    public class ServiceLogger : IServiceLogger {
        ...
        public ServiceLogger(IEnvironmentProvider environment, IOptions<ServiceLoggingOption> loggingOptions, string logName) {
        ...
        private string GetRollingFilePath(string basePath, string shortDate) {
            var maxSize = _options.MaxFileSizeInMB * 1024 * 1024;
        ...
        private void CleanupOldLogs(string basePath) {
            try {
                ...
                foreach (var file in files) {
                    ...
                    if (info.CreationTime < DateTime.Now.AddDays(-_options.RetentionDays)) {
                        info.Delete();
                    }
                }
            } catch (Exception ex) {
