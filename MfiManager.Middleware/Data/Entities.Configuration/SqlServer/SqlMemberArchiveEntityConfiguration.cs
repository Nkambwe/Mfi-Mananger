...
namespace MfiManager.Middleware.Data.Entities.Configuration.SqlServer {
    ...
    public class SqlMemberArchiveEntityConfiguration {

        public static void Configure(EntityTypeBuilder<MemberArchive> builder) {
            builder.ToTable("TBL_MFI_ARCHIVE_MEMBER");
            builder.HasKey(p => p.Id);
            builder.Property(p => p.Id).HasColumnName("id");
            builder.Property(p => p.MemberId).HasColumnName("old_id");
            ...
            builder.Property(p => p.ClientCode).HasColumnName("member_number").HasColumnType("NVARCHAR(10)").IsRequired();
            ...
            builder.HasOne(p => p.Title).WithMany(e => e.MemberArchives).HasForeignKey(e => e.TitleId);
            builder.HasOne(p => p.Village).WithMany(e => e.MemberArchives).HasForeignKey(e => e.VillageId);
            builder.HasOne(p => p.Group).WithMany(e => e.MemberArchive).HasForeignKey(e => e.GroupId);
            builder.HasOne(p => p.Profession).WithMany(e => e.MemberArchive).HasForeignKey(e => e.ProfessionId);
            builder.HasOne(p => p.Nationality).WithMany(e => e.MemberArchive).HasForeignKey(e => e.NationalityId);
        }
    }
}
