...
namespace MfiManager.Middleware.Data.Entities.Configuration.SqlServer {
    ...
    public class SqlIndividualArchiveEntityConfiguration {

        public static void Configure(EntityTypeBuilder<IndividualArchive> builder) {
            builder.ToTable("TBL_MFI_ARCHIVE_INDIVIDUAL");
            builder.HasKey(p => p.Id);
            builder.Property(p => p.Id).HasColumnName("id");
            builder.Property(p => p.PersonId).HasColumnName("old_id");
            ...
            builder.Property(p => p.Code).HasColumnName("client_number").HasColumnType("NVARCHAR(10)").IsRequired();
            ...
            builder.HasOne(p => p.Branch).WithMany(e => e.IndividualArchives).HasForeignKey(e => e.BranchId);
            builder.HasOne(p => p.Title).WithMany(e => e.IndividualArchives).HasForeignKey(e => e.TitleId);
            builder.HasOne(p => p.Village).WithMany(e => e.IndividualArchives).HasForeignKey(e => e.VillageId);
            builder.HasOne(p => p.Profession).WithMany(e => e.IndividualArchives).HasForeignKey(e => e.ProfessionId);
            builder.HasOne(p => p.Nationality).WithMany(e => e.IndividualArchives).HasForeignKey(e => e.NationalityId);
        }
    }
