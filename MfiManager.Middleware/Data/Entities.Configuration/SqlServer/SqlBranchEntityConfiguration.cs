...
namespace MfiManager.Middleware.Data.Entities.Configuration.SqlServer {
    ...
    public class SqlBranchEntityConfiguration {
        public static void Configure(EntityTypeBuilder<Branch> builder) {
            ...
            builder.HasMany(mp => mp.LockedFiles).WithOne(l => l.Branch).HasForeignKey(l => l.BranchId);
            builder.HasMany(mp => mp.Configurations).WithOne(c => c.Branch).HasForeignKey(c => c.BranchId);
            builder.HasMany(mp => mp.Series).WithOne(s => s.Branch).HasForeignKey(s => s.BranchId);
