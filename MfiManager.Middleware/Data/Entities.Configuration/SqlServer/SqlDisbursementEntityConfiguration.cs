...
namespace MfiManager.Middleware.Data.Entities.Configuration.SqlServer {
    public class SqlDisbursementEntityConfiguration {
        ...
        public static void Configure(EntityTypeBuilder<Disbursement> builder) {
            ...
            builder.HasOne(bc => bc.IndividualLoan).WithMany(p => p.Disbursements).HasForeignKey(bc => bc.IndividualLoanId).OnDelete(DeleteBehavior.Cascade);
            builder.Property(p => p.BusinessLoanId).HasColumnName("business_lnr_id").IsRequired(false);
            builder.HasOne(bc => bc.BusinessLoan).WithMany(p => p.Disbursements).HasForeignKey(bc => bc.BusinessLoanId).OnDelete(DeleteBehavior.Cascade);
            builder.Property(p => p.GroupLoanId).HasColumnName("group_lnr_id").IsRequired(false);
            builder.HasOne(bc => bc.GroupLoan).WithMany(p => p.Disbursements).HasForeignKey(bc => bc.GroupLoanId).OnDelete(DeleteBehavior.Cascade);
        }
    }
