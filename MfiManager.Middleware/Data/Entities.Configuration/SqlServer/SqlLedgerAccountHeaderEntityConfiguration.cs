...
namespace MfiManager.Middleware.Data.Entities.Configuration.SqlServer {
    public class SqlLedgerAccountHeaderEntityConfiguration {
        public static void Configure(EntityTypeBuilder<LedgerAccountHeader> builder) {
            builder.ToTable("TBL_MFI_LEDGER_ACC_HEADER");
            builder.HasKey(p => p.Id);
            builder.Property(p => p.Id).HasColumnName("id");
            builder.Property(p => p.ParentHeader).HasColumnName("parent_account").HasColumnType("NVARCHAR(10)").IsRequired();
            ...
            builder.HasMany(m => m.LedgerAccounts).WithOne(o => o.LedgerAccountHeader).HasForeignKey(mp => mp.LedgerAccountHeaderId);
            builder.HasMany(m => m.LedgerAccountTotalLabels).WithOne(o => o.LedgerAccountTotalLabel).HasForeignKey(mp => mp.LedgerAccountTotalLabelId);
        }
    }
