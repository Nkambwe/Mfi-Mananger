...
namespace MfiManager.Middleware.Data.Entities.Configuration.SqlServer {
    public class SqlClientApprovalEntityConfiguration {

        public static void Configure(EntityTypeBuilder<ClientApproval> builder) {
            builder.ToTable("TBL_MFI_CLIENT_APPROVAL");
            ...
            builder.HasOne(p => p.Individual).WithMany(e => e.Approvals).HasForeignKey(e => e.Individual);
            builder.HasOne(p => p.Group).WithMany(e => e.Approvals).HasForeignKey(e => e.GroupId);
            builder.HasOne(p => p.Business).WithMany(e => e.Approvals).HasForeignKey(e => e.BusinessId);
            builder.HasOne(p => p.Member).WithMany(e => e.Approvals).HasForeignKey(e => e.MemberId);
        }
    }
