...
namespace MfiManager.Middleware.Data.Entities.Configuration.SqlServer {
    ...
    public class SqlApplicationNotesEntityConfiguration {
        ...
        public static void Configure(EntityTypeBuilder<ApplicationNotes> builder) {
            ...
            builder.HasOne(bc => bc.IndividualLoan).WithMany(p => p.ApprovalNotes).HasForeignKey(bc => bc.IndividualLoanId).OnDelete(DeleteBehavior.Cascade);
            builder.Property(p => p.BusinessLoanId).HasColumnName("business_lnr_id").IsRequired(false);
            builder.HasOne(bc => bc.BusinessLoan).WithMany(p => p.ApprovalNotes).HasForeignKey(bc => bc.BusinessLoanId).OnDelete(DeleteBehavior.Cascade);
            builder.Property(p => p.GroupLoanId).HasColumnName("group_lnr_id").IsRequired(false);
            builder.HasOne(bc => bc.GroupLoan).WithMany(p => p.ApprovalNotes).HasForeignKey(bc => bc.GroupLoanId).OnDelete(DeleteBehavior.Cascade);
        }
    }
}
